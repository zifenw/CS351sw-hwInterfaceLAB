...
void* mm_malloc(size_t size) {
  ...
  ptr_free_block = search_free_list(req_size);
    //Obtain more heap space of at least req_size
...
//Decide whether to split the found block into two parts: one part is allocated to the current request. The remaining part will be reused as a free block.
//If there is enough remaining space to form a new free block (at least MIN_SLOCK_SIZE), the block needs to be split.
if (block_size - req_size >= MIN_BLOCK_SIZE) {
    // Set the block's size and tags
    ptr_free_block->size_and_tags = req_size | preceding_block_use_tag; //Set the block size to req_Size + retain information on whether the previous block has been used
    ptr_free_block->size_and_tags = ptr_free_block->size_and_tags | TAG_USED; //Set Tag Used for this block

    // Set the remainding free block's preceding used tag
    // Calculate the starting position of the new free block using POINTER_ADD. Set the size of its header and boundary markers to blockSize reqSize, and set TAG_PRECEDING-USED
    //Writes the header of the remaining free block created after splitting.
    *((size_t*) UNSCALED_POINTER_ADD(ptr_free_block, req_size)) = (block_size - req_size) | TAG_PRECEDING_USED;
    // Set the preceding used tag in boundary tag of the remainding free block
    //Writes the footer of the same remaining free block.
    *((size_t*) UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE)) = (block_size - req_size) | TAG_PRECEDING_USED;
    ...
    // The newly formed free blocks need to be added to the free list for subsequent allocation. Call insertFreeBlock to insert the pointer to the BlockInfo structure of the new idle block into the free list.
    insert_free_block((block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size));
  } else {
    // If the found block is exactly equal to or less than reqSize+MIN_SLOCK_SIZE, there is no need to split it. Directly allocate the entire block to the request and update its metadata.
    // Get the pointer to the following block
    block_info* remain_block = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, block_size);
    // Set the following block's previous used tag
    remain_block->size_and_tags = remain_block->size_and_tags | TAG_PRECEDING_USED;
